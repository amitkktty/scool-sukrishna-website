...
$q0="select count(*) as totalrecord from exam_detail ";
...
// End:    Database limit query dispaly code	i.e. start_from, to 
	...
	if($page_number < $pagination_button AND ($last_page == $pagination_button OR $last_page > $pagination_button))
	{
		for($i=1; $i<=$pagination_button; $i++)
		{
			...
			{
				$pagination .='<li class="">
								<a id="'.$i.'" class="mx-1 border border-dark   badge badge-info rounded p-2 bold text-white" href="Exam-List?page='.$i.'">'.$i.'</a></li>
								';
			}
			else
			{
				$pagination .='<li class="">
								<a id="'.$i.'" class=" border border-dark mx-1   badge bg-teal rounded p-2" href="Exam-List?page='.$i.'">'.$i.'</a>
								</li>';
			}
		}
	...
	else if(($page_number >= $pagination_button) AND ($last_page > $pagination_button))
	{
		if(($page_number+$half) >= $last_page)
		{
			...
			for($i=($last_page-$pagination_button)+1; $i<=$last_page; $i++)
			{
				...
				{
					$pagination .='<li class="">
									<a id="'.$i.'" class=" border border-dark mx-1   badge bg-teal rounded p-2" href="Exam-List?page='.$i.'">'.$i.'</a></li>
									';
				}
			}	
		}
