...
if(empty($state_id))
{ 
  echo "<script>window.location.href='State';</script>";
}
else
{
	$sql="select * from state_data where state_id=$state_id";
...
if (isset($_POST['submit'])) 
{
	...
	if(empty($errors))
	{
	 ...
	 $q1="select count(*) as totalfound from state_data where country_id='$country_id' AND statename='$state_name' AND state_id != $state_id"; 
	 ...
	 {	   
	  ...
	  if(!empty($_FILES ['mapfile']['name']))
	  {
		...
		$q = "update state_data set country_id = '$country_id', statename = '$state_name', state_logo = '$logo_file_name', state_description = '$description', state_map = '$map_file_name'  where state_id=$state_id";
...
else
{
	...
	$old_state_name  	= $rr->statename;
	$state_name  	= $rr->statename;
	$oldlogofile  		= $rr->state_logo; 
	$oldmapfile  		= $rr->state_map; 
	$aboutcountry  = $rr->state_description; 
	 
	$sql1="select * from state_data_extra where country_id='$country_id' AND state_name='$state_name'";
	$sql_qr1=mysqli_query($conn, $sql1);
	$statefound=mysqli_num_rows($sql_qr1);	
    ...
    <section >
	  ...
	  <div class="container-fluid px-4 ">
        <div class="col-md-11 mx-auto p-3 bg-white" style='box-shadow: rgba(0, 0, 0, 0.4) 0px 2px 4px, rgba(0, 0, 0, 0.3) 0px 7px 13px -3px, rgba(0, 0, 0, 0.2) 0px -3px 0px inset;'>
			...
			<div>
			<form action="State-Modify?id=<?php echo $state_id;?>" method="POST" enctype="multipart/form-data">
				<div class="row bold">
				  ...
				  <div class="col-md-6 my-2	">State Logo :
					...
					<input type="file" class='form-control form-control-sm' name='logofile'/><input type="hidden" class='form-control form-control-sm' value='<?php echo $oldlogofile;?>' readonly name='oldlogofile'/>			  
					</div>
				  ...
				  <textarea name="aboutcountry"  style='resize:none;' class='form-control form-control-sm' rows="5"><?php echo $aboutcountry;?></textarea>		  
				  </div> 
				  <div class="col-md-12">
					<br>
					<div id="containerss">
