...
if (isset($_POST['submit'])) 
{
   ...
   if($_FILES["image"]["name"])
   {
    $name = $_FILES["image"]["name"];
       ...
       $sql="update testimonial set category='$category',name='$names',photo='$image',comment='$comment',date='$date' where testim_id='$testim_id'";                 

                      }
                      else
                      {
       $sql="update testimonial set category='$category',name='$names',comment='$comment',date='$date' where testim_id='$testim_id'";
                      }
      ...
      <div class="container-fluid text-uppercase ">
	   <div class="row bg-white p-2" style='box-shadow: rgba(0, 0, 0, 0.1) 0px 4px 6px -1px, rgba(0, 0, 0, 0.06) 0px 2px 4px -1px;'>
	    ...
	    <div>
		  ...
		  <h6 class='text-primary bold mt-n3'>Update Testimonial</h6></h3>
		</div>	   
	   </div><br>		
      </div>
...
$sql=mysqli_query($conn,"select * from testimonial where testim_id='$testim_id'");
...
$rows=mysqli_fetch_object($sql)
	?>		
           ...
           while($row=mysqli_fetch_object($sql))
           {
				  ...
				  <input type="text" class='form-control form-control-sm'  required='' value="<?php echo $rows->name;?>"  name='name' />				  
				  </div>
				  ...
				  <textarea name="comment" id='editor' class='form-control form-control-sm' rows="5"  ><?php echo $rows->comment;?></textarea>			  
				  </div>
