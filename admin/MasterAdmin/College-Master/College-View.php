...
echo $college_name = $_SESSION['college_name']; 
...
if(isset($_POST['submit']))
{
	...
	if(!empty($college_id))
	{
		$q2 = "delete from college_view_extra where college_id ='$college_id'";
		...
		foreach ($_POST['title'] as $key => $value)
		{
			...
			{
				$q3 = "insert into college_view_extra values(null,'$college_id','$title_head','$description','$currentdt','','','','')";
				mysqli_query($conn, $q3);
			}
		}
        echo"<script>window.location.href='finalSubmitData'</script>"; 		
	}
	else
	{
     echo "<script>alert('Please seelct college. ')</script>";
	}	
}
?>
			...
			<div class="row bold">
				<div class="col-md-12 my-2">College Name * : <?php echo $errors[college_name]; ?>
					<select required='' id='college_name' name="college_name" onchange='setCollege(this.value)' class='form-control form-control-sm chosen-select py-2'>
						...
						<?php 
						$q10 = "select * from college_data where college_type='College' order by college_name ";
					...
					<?php echo $_SESSION['college_full_name']?>
...
<div class="col-md-12 mx-auto p-3 bg-white sd-2" >
	...
	<h5 class='text-center bold bg-teal mt-n2'>Add Seat Matrix</h5>
            ...
            <tr>
                <th>Course</th>
                 <th>Number of Seat</th>
                  <th>Fees Tution (in Lakh)</th>
            </tr>
        </thead>
        <tbody>
            <?php
            $sql=mysqli_query($conn,"select * from course_name order by course_name");
            while($res=mysqli_fetch_object($sql))
            {
            ?>
            <tr>
               <td style="width:20%"><?php echo $res->course_name;?></td>
...
<!-- ----------- Add College Gallery ------------- -->
	...
	<div>
	<form action="College-View" method="post">
	 <div id="containerss">
	 <?php
	 ...
	 else
	 {
		...
		$sql1="select * from college_view_extra where college_id='$college_id'";
		$sql_qr1=mysqli_query($conn, $sql1);
		$viewfound=mysqli_num_rows($sql_qr1);
		if($viewfound > 0)
		{
			$oldid=1;	
		   ...
		   <fieldset class="mb-1">
			  <div class="form-group form-group-floating row">
				 <div class="col-12">
					<div class="position-relative"><textarea class="form-control form-control-outline id_value<?=$oldid;?>" placeholder="Section Description" name="description[]" id="description<?=$oldid;?>"><?= $description;?></textarea> </div>
				 </div>
			  </div>
		   </fieldset> 
	       </div> 
	 ...
	 <!--
	<a href="finalSubmitData" onclick="return confirm('Are you sure?')" class='btn btn-sm bg-teal px-3 bold text-white'>Final Submit</a> -->
	<button name='submit' onclick="return confirm('Are you sure?')" class='btn btn-sm bg-teal px-3 bold text-white'>Final Submit</button>
